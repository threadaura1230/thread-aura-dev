...
export default function MobileMenu({
  ...
  const handleScroll = (e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    e.preventDefault();
    onClose();
    // Immediately remove scroll lock so that window.scrollTo works
    document.body.style.overflow = "";
  ...
  return (
    <>
      ...
      <div
        ref={menuRef}
        className={`fixed inset-y-0 left-0 z-[60] flex h-full w-80 max-w-[85vw] flex-col border-r border-black/[0.06] bg-[#F1EFE7] transition-transform duration-300 ease-in-out md:hidden ${
      ...
      >
        ...
        {/* Search Box Section - kept outside of scrollable container */}
        <div ref={searchContainerRef} className="px-5 py-4 border-b border-black/[0.05] relative bg-black/[0.005]">
          ...
          {/* Search Results Overlay (Absolute Floating) */}
          {showResults && (
            <div className="absolute left-5 right-5 mt-2 z-50 rounded-xl bg-white/98 backdrop-blur-md border border-black/[0.08] shadow-xl p-4 max-h-[350px] overflow-y-auto font-sans">
              {searchLoading ? (
